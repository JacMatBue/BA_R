...
election_MLT <- ParlGov_election %>%
  filter(country_name == "Malta", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_MLT <- ParlGov_cabinet %>%
  filter(country_name == "Malta", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_MLT <- election_MLT %>%
  left_join(cabinet_MLT[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
MLT_pop <- left_join(MLT_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
