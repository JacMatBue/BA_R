...
election_FIN <- ParlGov_election %>%
  filter(country_name == "Finland", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_FIN <- ParlGov_cabinet %>%
  filter(country_name == "Finland", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_FIN <- election_FIN %>%
  left_join(cabinet_FIN[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
FIN_pop <- left_join(FIN_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
