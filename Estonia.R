...
election_EST <- ParlGov_election %>%
  filter(country_name == "Estonia", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_EST <- ParlGov_cabinet %>%
  filter(country_name == "Estonia", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_XXX and cabinet_XXX

merged_EST <- election_EST %>%
  left_join(cabinet_EST[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
EST_pop <- left_join(EST_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
