...
election_NLD <- ParlGov_election %>%
  filter(country_name == "Netherlands", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_NLD <- ParlGov_cabinet %>%
  filter(country_name == "Netherlands", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_NLD <- election_NLD %>%
  left_join(cabinet_NLD[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
NLD_pop <- left_join(NLD_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
