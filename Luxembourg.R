...
election_LUX <- ParlGov_election %>%
  filter(country_name == "Luxembourg", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_LUX <- ParlGov_cabinet %>%
  filter(country_name == "Luxembourg", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_LUX <- election_LUX %>%
  left_join(cabinet_LUX[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
LUX_pop <- left_join(LUX_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
