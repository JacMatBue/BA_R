...
election_CZE <- ParlGov_election %>%
  filter(country_name == "Czech Republic", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_CZE <- ParlGov_cabinet %>%
  filter(country_name == "Czech Republic", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_CZE <- election_CZE %>%
  left_join(cabinet_CZE[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
CZE_pop <- left_join(CZE_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
