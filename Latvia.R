...
election_LVA <- ParlGov_election %>%
  filter(country_name == "Latvia", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_LVA <- ParlGov_cabinet %>%
  filter(country_name == "Latvia", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_LVA <- election_LVA %>%
  left_join(cabinet_LVA[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
LVA_pop <- left_join(LVA_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
