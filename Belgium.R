...
election_BEL <- ParlGov_election %>%
  filter(country_name == "Belgium", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_BEL <- ParlGov_cabinet %>%
  filter(country_name == "Belgium", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_BEL <- election_BEL %>%
  left_join(cabinet_BEL[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
BEL_pop <- left_join(BEL_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
