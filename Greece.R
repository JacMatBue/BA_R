...
election_GRC <- ParlGov_election %>%
  filter(country_name == "Greece", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_GRC <- ParlGov_cabinet %>%
  filter(country_name == "Greece", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_GRC <- election_GRC %>%
  left_join(cabinet_GRC[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
GRC_pop <- left_join(GRC_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
