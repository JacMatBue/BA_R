...
election_ISL <- ParlGov_election %>%
  filter(country_name == "Iceland", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_ISL <- ParlGov_cabinet %>%
  filter(country_name == "Iceland", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_ISL <- election_ISL %>%
  left_join(cabinet_ISL[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
ISL_pop <- left_join(ISL_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
