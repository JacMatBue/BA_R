...
election_SVK<- ParlGov_election %>%
  filter(country_name == "Slovakia", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_SVK <- ParlGov_cabinet %>%
  filter(country_name == "Slovakia", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_SVK <- election_SVK %>%
  left_join(cabinet_SVK[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
SVK_pop <- left_join(SVK_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
