...
election_BGR <- ParlGov_election %>%
  filter(country_name == "Bulgaria", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_BGR <- ParlGov_cabinet %>%
  filter(country_name == "Bulgaria", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_BGR <- election_BGR %>%
  left_join(cabinet_BGR[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
BGR_pop <- left_join(BGR_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
