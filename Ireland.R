...
election_IRL <- ParlGov_election %>%
  filter(country_name == "Ireland", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_IRL <- ParlGov_cabinet %>%
  filter(country_name == "Ireland", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_IRL <- election_IRL %>%
  left_join(cabinet_IRL[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
IRL_pop <- left_join(IRL_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
