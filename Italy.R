...
election_ITA <- ParlGov_election %>%
  filter(country_name == "Italy", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_ITA <- ParlGov_cabinet %>%
  filter(country_name == "Italy", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_ITA <- election_ITA %>%
  left_join(cabinet_ITA[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
ITA_pop <- left_join(ITA_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
