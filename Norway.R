...
election_NOR <- ParlGov_election %>%
  filter(country_name == "Norway", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_NOR <- ParlGov_cabinet %>%
  filter(country_name == "Norway", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_NOR <- election_NOR %>%
  left_join(cabinet_NOR[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
NOR_pop <- left_join(NOR_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
