...
election_HRV <- ParlGov_election %>%
  filter(country_name == "Croatia", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_HRV <- ParlGov_cabinet %>%
  filter(country_name == "Croatia", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_HRV <- election_HRV %>%
  left_join(cabinet_HRV[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
## adjust dataset (remove ethnic and one seat)

HRV_4 <- HRV_3 %>%
  filter(!(party_name %in% c("ethnic", "one seat", "no party affiliation", "abroad")))
...
HRV_pop <- left_join(HRV_4, selected_variables, by=c("party_id"="parlgov_id")) %>%
