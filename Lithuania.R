...
election_LTU <- ParlGov_election %>%
  filter(country_name == "Lithuania", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_LTU <- ParlGov_cabinet %>%
  filter(country_name == "Lithuania", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_LTU <- election_LTU %>%
  left_join(cabinet_LTU[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
LTU_pop <- left_join(LTU_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
