...
election_SWE <- ParlGov_election %>%
  filter(country_name == "Sweden", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_SWE <- ParlGov_cabinet %>%
  filter(country_name == "Sweden", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_SWE <- election_SWE %>%
  left_join(cabinet_SWE[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
SWE_pop <- left_join(SWE_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
