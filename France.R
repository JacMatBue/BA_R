...
election_FRA <- ParlGov_election %>%
  filter(country_name == "France", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_FRA <- ParlGov_cabinet %>%
  filter(country_name == "France", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_FRA <- election_FRA %>%
  left_join(cabinet_FRA[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
FRA_pop <- left_join(FRA_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
