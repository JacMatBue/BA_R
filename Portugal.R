...
election_PRT <- ParlGov_election %>%
  filter(country_name == "Portugal", as.numeric(substr(election_date, 1, 4)) >= 1998, election_type == "parliament", seats != 0)

cabinet_PRT <- ParlGov_cabinet %>%
  filter(country_name == "Portugal", as.numeric(substr(election_date, 1, 4)) >= 1998)


## merge election_AUT and cabinet_AUT 

merged_PRT <- election_PRT %>%
  left_join(cabinet_PRT[, c("election_id", "cabinet_party", "party_name", "cabinet_id", "prime_minister")], 
...
PRT_pop <- left_join(PRT_3, selected_variables, by=c("party_id"="parlgov_id")) %>%
